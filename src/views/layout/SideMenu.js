...
class SideNenu extends Component {
	...
	renderMenu = data => {
		return data.map(item => {
			...
			return (
				this.handleFilter(item.permission) && (
					<Menu.Item key={item.path} style={{height:'50px',lineHeight:'50px'}}>
	...
	render() {
		...
		return (
			<Sider trigger={null} collapsible collapsed={collapse.isCollapsed} theme={type} className="app-sider"  style={{
